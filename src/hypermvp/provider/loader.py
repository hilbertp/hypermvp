...
def load_provider_file(filepath):
    ...
    try:
        ...
        # Check if we have DELIVERY_DATE column and show date range
...
def load_provider_data(directory=None):
    ...
    import os
    import glob
    from hypermvp.config import PROVIDER_FILE_PATHS, PROVIDER_RAW_DIR
    ...
    for i, file_path in enumerate(file_paths, 1):
        ...
        try:
            ...
            # Get date range in the file if available
    ...
    if 'DELIVERY_DATE' in result.columns and not result.empty:
        ...
        logging.info(f"Total date range: {min_date} to {max_date}")
