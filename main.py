...
def main():
    ...
    # Pass as a list, we need this later because there so many 
    # different provider pool for a day but for now the code can't handle it 
    ...
    # Step 4: Save cleaned aFRR data to the 'processed' directory

    # Generate a unique filename using a timestamp
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    afrr_filename = f'cleaned_afrr_{timestamp}.csv'
    afrr_output_path = os.path.join(PROCESSED_DATA_DIR, afrr_filename)
